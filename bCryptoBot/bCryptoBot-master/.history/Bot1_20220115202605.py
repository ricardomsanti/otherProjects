...
HERO_KEYS = [ "ID",\
                ...
                "BOMB_RANGE",\
                "LIFE"]
dash_path = "C:\\Users\\QQ\\Google Drive\\CODE_DRIVE\\CRYPTO\\"
HEROE = {x:r.randint(1,10) for x in HERO_KEYS}

HEROES_LIST = [{x:r.randint(1,10) for x in HERO_KEYS} for x in range(15)]
df = pd.DataFrame(HEROES_LIST)
df.to_csv(dash_path + str(HEROES_LIST))
