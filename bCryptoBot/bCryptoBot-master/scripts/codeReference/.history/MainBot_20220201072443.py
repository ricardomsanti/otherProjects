...
class Cryptus:
    ...
    def mainPreConf(target=None):
        print("=====================================================")      
        key = input(target[0])
        record = p.confirm(title="SETUP", text=f"GRAVAR ALVO {str(target).replace('.self')}",button=["OK", "STOP"])
        if record == 'ok':
            for x in [3,2,1,"ok"]:
                t.sleep(1)
                print(x)
    ...
    def mainConf(self):
        ...
        BC_CRD = self.mainPreConf(target=self.BC_CRD)
        BLUE_BOX_CRD = self.mainPreConf(target=self.BLUE_BOX_CRD)
        BROWN_BOX_CRD = self.mainPreConf(target=self.BROWN_BOX_CRD)
        OPEN_CRD = self.mainPreConf(target=self.OPEN_CRD)
        PURPLE_BOX_CRD = self.mainPreConf(target=self.PURPLE_BOX_CRD)
        RA_CRD = self.mainPreConf(target=self.RA_CRD)
        TH_CRD = self.mainPreConf(target=self.TH_CRD)
        WA_CRD = self.mainPreConf(target=self.WA_CRD)
        WARRIOR_CRD = self.mainPreConf(target=self.WARRIOR_CRD)
        EX_CRD = self.mainPreConf(target=self.EX_CRD)
        CRD_LIST = self.mainPreConf(target=self.CRD_LIST)
        activeWindow = self.mainPreConf(target=self.activeWindow)
        ...
        key = input("BOTÃO VERDE SUPERIOR ESQUEDO\n")
        ...
        key = input("BOTÃO DO BONECO DE ARMADURA QUE ABRE A JANELA DE CONTROLE\n")     
        if key == "GRAVAR":
            for x in [3,2,1,"ok"]:
                print(x)
        ...
        key  = input("BOTÃO QUE COLOCA TODOS PARA TRABALHAR\n")      
        if key == "GRAVAR":
            for x in [3,2,1,"ok"]:
                print(x)
        ...
        key  = input("BOTÃO CENTRAL TREASURE HUNT\n")
        if key == "GRAVAR":
            for x in [3,2,1,"ok"]:
                print(x)
        ...
        key =input("BOTÃO QUE COLOCA TODOS PARA DESCANSAR\n")  
        if key == "GRAVAR":
            for x in [3,2,1,"ok"]:
                print(x)
            ...
            t.sleep(3)
            self.aim.runAim()
        key  = input("EXEMPLO BAÚ ROXO\n") 

        key =  input("BOTÃO DE SAÍDA\n")    
        if key == "GRAVAR":
            for x in [3,2,1,"ok"]:
                print(x)
            self.EX_BUTTON =  p.position()
            t.sleep(3)
            self.aim.runAim()
